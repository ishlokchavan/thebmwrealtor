    ...
    <section className="mx-auto max-w-6xl px-4 py-14 sm:py-20">
      ...
      <div className="mt-10 grid gap-4 md:grid-cols-3">
        {testimonials.map((t) => (
          <figure
            key={t.name}
            className="flex flex-col rounded-2xl border border-slate-200/70 bg-white p-6 shadow-soft"
          >
            ...
            <figcaption className="mt-5 flex items-center gap-3 border-t border-slate-100 pt-4">
              <span className="flex h-10 w-10 items-center justify-center rounded-full bg-night-950 text-sm font-bold text-gold-300">
                {t.initials}
              </span>
              <span>
