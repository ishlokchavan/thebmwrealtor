    ...
    <section className="mx-auto max-w-6xl px-4 py-14 sm:py-20">
      <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <div className="flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 shadow-soft">
          <span className="flex">
            {[0, 1, 2, 3, 4].map((i) => (
              <StarIcon key={i} className="h-4 w-4 text-bmw-500" />
            ))}
          </span>
      ...
      <div className="mt-10 grid gap-4 md:grid-cols-3">
        {testimonials.map((t) => (
          ...
          >
            <QuoteIcon className="h-8 w-8 text-bmw-200" />
            <blockquote className="mt-3 flex-1 text-[15px] leading-relaxed text-slate-700">
              &ldquo;{t.quote}&rdquo;
            </blockquote>
            <figcaption className="mt-5 flex items-center gap-3 border-t border-slate-100 pt-4">
              <span className="flex h-10 w-10 items-center justify-center rounded-full bg-carbon-950 text-sm font-bold text-bmw-300">
                {t.initials}
              </span>
              <span>
