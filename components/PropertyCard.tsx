  ...
  return (
    <div className={className}>
      <div className="w-full max-w-[280px] overflow-hidden rounded-2xl bg-white shadow-float">
        <span className="m-stripe block h-1 w-full" aria-hidden="true" />
        <div className="p-3">
          {/* Photo area */}
          <div className="relative h-36 overflow-hidden rounded-xl bg-carbon-900">
            <div
              ...
              style={{
                backgroundImage:
                  "radial-gradient(120% 100% at 20% 0%, #123566 0%, transparent 60%), linear-gradient(160deg, #12161f 0%, #0a0c10 100%)",
              }}
            />
            <svg viewBox="0 0 280 100" className="absolute bottom-0 left-0 w-full" aria-hidden="true">
              {[
                ...
                <rect key={i} x={x} y={y} width={w} height={h} rx="1" fill="#0a0c10" opacity={0.7 + (i % 2) * 0.2} />
              ))}
            </svg>
            <span className="absolute left-3 top-3 rounded bg-bmw-600 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-white">
              For Sale
            </span>
            <span className="absolute right-3 top-3 flex items-center gap-1 rounded bg-white/90 px-2 py-1 text-[10px] font-bold text-carbon-900">
              <StarIcon className="h-3 w-3 text-bmw-500" /> 4.9
            </span>
          </div>
          {/* Details */}
          <div className="px-1 pb-1 pt-3">
            ...
            <p className="mt-0.5 flex items-center gap-1 text-xs text-slate-500">
              <MapPinIcon className="h-3.5 w-3.5 text-bmw-500" /> Sector 78, Noida
            </p>
